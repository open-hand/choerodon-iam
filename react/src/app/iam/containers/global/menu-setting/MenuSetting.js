...
import { Output, Form as ProForm, IconPicker } from 'choerodon-ui/pro';
...
export default class MenuSetting extends Component {
  ...
  getDetailDom() {
    ...
    return (
      <div className="detailForm">
        <ProForm labelLayout="horizontal" labelAlign="left">
          <Output value={name} label="菜单名称" />
          <Output value={code} label="菜单编码" />
          <Output value={level} label="菜单层级" />
          {/* eslint-disable-next-line camelcase */}
          <Output value={__parent_name__} label="所属根目录" />
        </ProForm>
        ...
        >
          {getFieldDecorator('icon', {
            ...
            <IconPicker
              ...
              dropdownStyle={{ width: 310, minWidth: 310 }}
              showArrow
              showAll
            />,
          )}
  ...
  checkDraggable(record) {
    ...
    if (record.type === 'tab') return false;
    // console.log('判断是否能拖拽', record, !dragData || (dragData !== record && !isChild(dragData, record)));
  ...
  checkDropBesides(record) {
    ...
    return dragData && (
      /* eslint-disable-next-line */
      record.__level__ === 0 ? record.type !== 'menu_item' :
