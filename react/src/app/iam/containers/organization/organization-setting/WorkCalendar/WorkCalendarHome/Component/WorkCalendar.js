...
export default function WorkCalendar(props) {
   ...
   * @returns {*}
   */
  function dateRender(current, now) {
    // 渲染当前页面可见月数据
  ...
  function onSelectDate(date, source) {
  ...
  function renderTag(title, color = '#000', fontColor = '#FFF', text) {
    return (
  ...
  function renderFooter() {
    return (
      <div>
        ...
        >
          {renderTag('起始日/结束日', '#3F51B5', '#FFF', 'N')}
          {renderTag('工作日', '#F5F5F5', '#000', 'N')}
          {renderTag('休息日', '#FEF3F2', '#EF2A26', 'N')}
          {renderTag('法定节假日补班', '#000', '#FFF', '班')}
          {renderTag('法定节假日', '#EF2A26', '#FFF', '休')}
        </div>
      </div>
    );
  }
